...
function renderShopView(activeFilter) {
  ...
  document.getElementById('main-content').innerHTML = [
    ...
    '    <div id="shop-promo-slot"></div>',
    '  </div>',
    '</div>',
    '<div id="shop-cart-slot" style="max-width:1100px;margin:16px auto 0;padding:0 1rem;"></div>',
    '<section class="section">',
    '  <div class="container">',
...
function renderProductPage(search) {
  ...
  document.getElementById('main-content').innerHTML = [
    ...
    '        <img src="' + product.image + '" alt="' + product.name + '" onerror="this.onerror=null;this.src=\'https://placehold.co/600x600/0d1b2a/ffffff?text=' + encodeURIComponent(product.category) + '\'" />',
    '      </div>',
